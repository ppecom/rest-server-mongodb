...
UserSchema.methods.setPassword = async function (v) {
	const hash = await bcrypt.hash(v, 10);
	this.hashedPassword = hash;
};

UserSchema.methods.checkPassword = async function (v) {
	const result = await bcrypt.compare(v, this.hashedPassword);
	return result;
};
